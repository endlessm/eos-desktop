...
const IconGrid = new Lang.Class({
    ...
    getHeightForRows: function(nRows) {
        ...
        return Math.max(1, nRows) * this._getVItemSize() + totalSpacing + this.topPadding + this.bottomPadding;
    },
    ...
    _allocate: function (grid, box, flags) {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (columnIndex == 0) {
                y += this._getVItemSize() + this._spacing;
                x = box.x1 + leftEmptySpace + this.leftPadding;
            } else {
                x += this._getHItemSize() + this._spacing;
            }
        }
    ...
    _calculateChildBox: function(child, x, y, box) {
        ...
        let width = Math.max(this._getHItemSize(), childNaturalWidth);
        let childXSpacing = Math.max(0, width - childNaturalWidth) / 2;
        let height = Math.max(this._getVItemSize(), childNaturalHeight);
    ...
    _computeLayout: function (forWidth) {
        ...
        while ((this._colLimit == null || nColumns < this._colLimit) &&
            ...
            usedWidth += this._getHItemSize() + this._getSpacing;
            nColumns += 1;
        }
