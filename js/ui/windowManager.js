...
const WindowManager = new Lang.Class({
    ...
    _handleRotateBetweenPidWindows: function(proxy, sender, [src, dst]) {
        ...
        const idForFromApp = actorForFromPid ? actorForFromPid.get_gtk_application_id() : null;
        const idForToApp = actorForToPid ? actorForToPid.get_gtk_application_id() : null;
    ...
    _updateReadyRotateAnimationsWith: function (window) {
        ...
        this._pendingRotateAnimations = this._pendingRotateAnimations.filter(function(animationSpec) {
            ...
            if (unsatisfiedPids == 0) {
                log("Would trigger rotate from " + animationSpec.src.window.get_meta_window() +
                    " to " + animationSpec.dst.window.get_meta_window());
