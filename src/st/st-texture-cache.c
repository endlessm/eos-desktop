                   ...
                   GCancellable *cancellable)
{
  ...
  stream = g_file_read (data->gfile, NULL, &error);
  if (stream == NULL)
    {
  ...
  pix = gdk_pixbuf_new_from_stream (G_INPUT_STREAM (stream), NULL, &error);
                                                 ...
                                                 GError         **error)
{
  ...
  if (texdata == NULL)
    {
      pixbuf = impl_load_pixbuf_file (file, available_width, available_height, error);
      if (!pixbuf)
        goto out;
